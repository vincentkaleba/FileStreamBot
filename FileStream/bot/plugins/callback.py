...
async def cb_data(bot, update: CallbackQuery):
    ...
    #---------------------[ MY FILES CMD ]---------------------#
        ...
        caption= "**Cᴏɴғɪʀᴍᴇᴢ ǫᴜᴇ ᴠᴏᴜs ᴠᴏᴜʟᴇᴢ sᴜᴘᴘʀɪᴍᴇʀ ʟᴇ ғɪᴄʜɪᴇʀ**\n\n",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ᴏᴜɪ", callback_data=f"msgdelyes_{usr_cmd[1]}_{usr_cmd[2]}"), InlineKeyboardButton("ɴᴏɴ", callback_data=f"myfile_{usr_cmd[1]}_{usr_cmd[2]}")]])
    )
        ...
        await update.message.edit_caption(
        caption= "**Cᴏɴғɪʀᴍᴇᴢ ǫᴜᴇ ᴠᴏᴜs ᴠᴏᴜʟᴇᴢ sᴜᴘᴘʀɪᴍᴇʀ ʟᴇ ғɪᴄʜɪᴇʀ**\n\n",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ᴏᴜɪ", callback_data=f"msgdelpvtyes_{usr_cmd[1]}"), InlineKeyboardButton("ɴᴏɴ", callback_data=f"mainstream_{usr_cmd[1]}")]])
    )
        ...
        await update.message.edit_caption(
            caption="Tᴏᴛᴀʟ ᴅᴇ ғɪᴄʜɪᴇʀs : {}".format(total_files),
        ...
        await update.answer(f"Eɴᴠᴏɪ ᴅᴜ ғɪᴄʜɪᴇʀ {file_name}")
...
async def gen_file_list_button(file_list_no: int, user_id: int):
    ...
    if not file_list:
        file_list.append(
                [InlineKeyboardButton("ᴠɪᴅᴇ", callback_data="N/A")])
    file_list.append([InlineKeyboardButton("ғᴇʀᴍᴇʀ", callback_data="close")])
...
async def gen_file_menu(_id, file_list_no, update: CallbackQuery):
    try:
        ...
        await update.answer("Fɪᴄʜɪᴇʀ ɴᴏɴ ᴛʀᴏᴜᴠᴇ́")
        return
    ...
    if "video" in file_type.lower():
        MYFILES_BUTTONS = InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton("ᴏʙᴛᴇɴɪʀ ʟᴇ ғɪᴄʜɪᴇʀ", callback_data=f"sendfile_{myfile_info['_id']}"),
                 InlineKeyboardButton("ʀᴇᴠᴏǫᴜᴇʀ ʟᴇ ғɪᴄʜɪᴇʀ", callback_data=f"msgdelete_{myfile_info['_id']}_{file_list_no}")],
                [InlineKeyboardButton("ʀᴇᴛᴏᴜʀ", callback_data="userfiles_{}".format(file_list_no))]
            ]
        )
    else:
        MYFILES_BUTTONS = InlineKeyboardMarkup(
    ...
    await update.edit_message_caption(
        caption="**ɴᴏᴍ ᴅᴜ ғɪᴄʜɪᴇʀ :** `{}`\n**ᴛᴀɪʟʟᴇ ᴅᴜ ғɪᴄʜɪᴇʀ :** `{}`\n**ᴛʏᴘᴇ ᴅᴇ ғɪᴄʜɪᴇʀ :** `{}`\n**ᴄʀᴇ́ᴇ́ ʟᴇ :** `{}`".format(myfile_info['file_name'],
...
async def delete_user_file(_id, file_list_no: int, update:CallbackQuery):
    ...
    try:
        ...
        await update.answer("Fɪᴄʜɪᴇʀ ᴅᴇ́ᴊᴀ̀ sᴜᴘᴘʀɪᴍᴇ́")
        return

    await db.delete_one_file(myfile_info['_id'])
    await db.count_links(update.from_user.id, "-")
    await update.message.edit_caption(
            caption= "**Fɪᴄʜɪᴇʀ sᴜᴘᴘʀɪᴍᴇ́ ᴀᴠᴇᴄ sᴜᴄᴄᴇ̀s !**" + update.message.caption.replace("Cᴏɴғɪʀᴍᴇᴢ ǫᴜᴇ ᴠᴏᴜs ᴠᴏᴜʟᴇᴢ sᴜᴘᴘʀɪᴍᴇʀ ʟᴇ ғɪᴄʜɪᴇʀ", ""),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ʀᴇᴛᴏᴜʀ", callback_data=f"userfiles_1")]])
        )
...
async def delete_user_filex(_id, update:CallbackQuery):
    ...
    await db.count_links(update.from_user.id, "-")
    await update.message.edit_caption(
            caption= "**Fɪᴄʜɪᴇʀ sᴜᴘᴘʀɪᴍᴇ́ ᴀᴠᴇᴄ sᴜᴄᴄᴇ̀s !**\n\n",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ғᴇʀᴍᴇʀ", callback_data=f"close")]])
        )